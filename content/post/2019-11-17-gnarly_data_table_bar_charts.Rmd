...
munged_football_data <- munged_football_data %>%
  ...
  .[order(date), roll_rating := frollmean(rating, n = 6, algo = "exact", align = "left"), by = "team"] %>%
  #remove unrated rows
  .[!is.na(roll_rating)] %>%
...
plotting_data <- engosccerdata::club_data %>%
...
football_gif <- animate(p_football, 
                        nframes = length(unique(plotting_data$year_frame)) * 0.75,
                        fps = 10)
```
