...
colour_data <- club_data %>%
  select(team, col1, col2) %>%
  filter(team %in% football_data$team)

plotting_data <- football_data[colour_data, on = "team"] %>%
...
p <- plotting_data %>%
  ggplot(aes(y = order_fill, x = roll_rating,)) +
  geom_tile(aes(x = roll_rating/2, width = roll_rating),
            alpha = 0.8, height = 0.9) +
  #scale_fill_manual(values = cols, guide = FALSE) +
  geom_text(aes(x = 0, label = paste(team, " ")),
            vjust = 0.2, hjust = 1) +
  geom_text(aes(label = sprintf("%1.2f",roll_rating)), hjust = 1) +
