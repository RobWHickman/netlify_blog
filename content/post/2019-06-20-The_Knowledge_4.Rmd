...
worst_national_team_players <- players_national_elo %>%
  ...
  select(year, player, url, team, apps, sub_apps, nation, ranking)

# show the 25 players with the worst ranking their nation had during that time
head(select(worst_national_team_players, -url), n = 25)
...
We can get a fuller answer by scraping the international data for the players who have played for low ranked nations.

```{r get_international_appearances, warning=FALSE,message=FALSE}
players_df <- worst_national_team_players %>%
  .[1:20,] %>%
  mutate(url = c(
...
low_ranked_appearances <- players_df %>%
...
p3 <- elo_data %>%
  ...
  geom_point() +
  geom_point(data = low_ranked_appearances, colour = "red", aes(x = match_date), size = 3) +
  geom_line() +
  # colour by football shirt colour
  scale_colour_manual(values = c("yellow", "darkblue", "lightblue", "green", "darkgreen", "darkgoldenrod")) +
  scale_x_date() +
  ggtitle("Lowest National Ranking of Premier League International Caps",
...
but https://www.11v11.com/players/christopher-wreh-4/ lists wreh playing on July 27th 1997 (1997/1998 season) at which point Liberia were ranked 110th in the world.
