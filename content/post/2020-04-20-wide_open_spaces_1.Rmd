...
Imagine two teams, I and J. Each of these has 11 players (hopefully) on the pitch at any time chasing after one ball. We want to know which team controls which parts of the pitch for each point in the match. As 'control' really only refers to "will player on my team get to a potential pass there first", pitch control is just looking at where players i,j,k... are going to be at time t -> time t + n.

The easiest way to start to approximate this is to imagine a set of players who never change direction, they only speed up or slow down (and possible reverse). E.g. a full back running up and down the wings like a rook in a chess game. Their location at t + 1 will be their current location plus the expected value of their velocity. 
...
p3 <- ggplot() +
  annotate_pitch(dimensions = pitch_statsbomb) +
  geom_tile(data = full_back_next_pos, aes(x = next_x, y = next_y, fill = Freq), alpha = 0.7, height = 10) +
  scale_fill_viridis_c(name = "confidence") +
  geom_segment(data = full_back_movement, aes(x = x, xend = next_x, y = y, yend = next_y), size = 2, arrow = arrow(length = unit(0.03, "npc"))) +
  geom_point(data = full_back_pos, aes(x = x , y = y), shape = 21, colour = "black", fill = "red", size = 5) +
  theme_pitch()
...
For every single p at time t pitch control (PC) is defined by equation 2

$$PC_{(p,t)} = \sigma \sum_{i} I_{(p,t)} - \sum_{j} I_{(p,t)}$$

where you sum across i (all the players on team I) and j (all the players on team J).

because this is a logistic function will be from 0 to 1 where <0.5 is control by team J and >0.5 is control by team I. E.g. if you drop a ball at place p at time t, if PC(p,t) is greater than 0.5, team I is likelier to get the ball, and viceversa for <0.5 (PC(p,t) is in the range 0->1).
...
All we need to do is find x, μ, and Σ, in that picture. Then we're going to use mvtnorm::dmvnorm to sample from it. If you run 
...
#will use game 1
get_tracking_data <- function(file, directory = "metrica-sports/sample-data", x_adj = 105, y_adj = 68) {
  #build url
...
tracking_data <- map_df(
  c("Sample_Game_1/Sample_Game_1_RawTrackingData_Away_Team.csv", "Sample_Game_1/Sample_Game_1_RawTrackingData_Home_Team.csv"),
  get_tracking_data) %>%
...
tracking_data <- tracking_data %>%
  #hacky fix for speeds?
  #dplyr::filter(time %in% seq(0, max(.$time), by = 0.2p)) %>%
