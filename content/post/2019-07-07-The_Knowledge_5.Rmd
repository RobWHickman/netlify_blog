...
league_cup <- readRDS("C:/Users/robwh/Documents/full_league_cup_data.rds") %>%
...
league_cup <- readRDS("C:/local/data/full_league_cup_data.rds") %>%
  select(date = Date, home, visitor, hgoal, vgoal)
...
scoring_data <- bind_rows(
  engsoccerdata::england %>%
    ...
    mutate(competition = "league"),
  engsoccerdata::facup %>%
    select(date = Date, home, visitor, hgoal, vgoal) %>%
    mutate(tier = NA, competition = "fa_cup"),
  engsoccerdata::englandplayoffs %>%
    select(date = Date, home, visitor, hgoal, vgoal, htier, vtier) %>%
    mutate(date = as.Date(date), tier = (htier+vtier)/2, 
...
team_locations <- readRDS("C:/Users/robwh/Documents/england_teams.rds") %>%
  st_set_crs(st_crs(st_crs("+init=epsg:4326")))
ground_locations <- readRDS("C:/Users/robwh/Documents/neutral_ground_locations.rds") %>%
  st_set_crs(st_crs(st_crs("+init=epsg:4326"))) %>%
  mutate_at(c("lat", "lon"), as.numeric)
matches <- readRDS("C:/Users/robwh/Documents/fa_cup_data.rds")
```
