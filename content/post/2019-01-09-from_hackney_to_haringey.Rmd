...
```{r brexit_differences, warning=FALSE,message=FALSE}
...
```{r}
dependent <- census_data_westminster %>%
...
lr_correlations <- map_df(dependent, function(x) {
  summary(lm(x ~ lr_margin)) %>%
    tidy() %>%
    filter(term != "(Intercept)")
})  %>%
  mutate(Code = names(census_data_westminster[2:ncol(census_data_westminster)])) %>%
...
```{r}
brexit <- lr_margin <- votes_data %>%
  left_join(constituency_names, by = "WSTnm") %>%
  .$brexit_hanretty

brexit_correlations <- map_df(dependent, function(x) {
  summary(lm(x ~ lr_margin)) %>%
    tidy() %>%
    filter(term != "(Intercept)")
})  %>%
...
p <- ggplot(pca, aes(x = PC1, y = PC2, label = WSTnm, colour = winner)) +
  geom_point() +
  geom_text() +
...
p <- ggplot(x, aes(x = distance, y = brexit_2016_difference)) +
    geom_point() +
    theme_minimal() +
    facet_wrap(~first_letter)
```
