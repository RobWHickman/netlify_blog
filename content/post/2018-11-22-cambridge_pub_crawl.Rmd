...
#first and sixty-ninth as defined earlier
x_journey_pubs <- pubs[x_random_numbers,]

#get all roads within the crow-distance btween each pub
x_local_roads <- roads[unlist(st_intersects(
  st_buffer(x_journey_pubs, max(as.numeric(st_distance(x_journey_pubs)))),
  roads
  )),]
...
(p4 <- ggplot() +
    ...
    geom_sf(data = x_journey_pubs, colour = "blue", alpha = 0.5, size = 2) +
    theme_minimal())
...
(p5 <- ggplot(data = shortest_tour) +
  ...
  geom_sf(colour = "red", size = 1.5, alpha = 0.7) +
  geom_sf(data = pubs, colour = "blue", size = 1.5) +
  theme_minimal() +
...
As it turns out, you'd want to start in Trumpington at The Lord Byron Inn and then get into central Cambridge where you dart around a lot before heading out to the north and finally the east at The Red Lion in Cherry Hinton. All together this script calculates a distances of 40km (to be 'exact') to visit every pub, which is actually kind of doable in a single day (if you forgo the drinking).
