...
player_data <- map_df(
  ...
  c("female_players.rds", "male_players.rds"), function(x) {
    data <- readRDS(paste0("../../static/files/fifa_data/", x)) %>%
...
best_teams <- 
  ...
  map_df(unique(paste(team_selection_dat$day, team_selection_dat$month, sep = "-")), 
         function(select_dob, data) {
           ...
           #probably a better way to write this but by now my brain was melting
...
p4 <- best_teams %>%
  ...
  geom_text(aes(label = gsub(" ", "\n", paste(name, version, sep = "-")), colour = ability), size = 5.5) +
