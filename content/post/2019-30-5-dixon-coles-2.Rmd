...
agoal_prob <- lapply(predictions$e_agoal, dpois, x = 0:max_goals) %>%
  lapply(., `names<-`, 0:max_goals) 

all_predictions <- map2_df(hgoal_prob, agoal_prob, function(home, away) {
    df <- outer(home, away) %>%
      as.data.frame() %>% 
      gather() %>% 
