...
Once again, we'll start by loading libraries. We also want the [sf](https://r-spatial.github.io/sf/articles/sf1.html) package that makes working with spatial data a bit cleaner.
...
Then we want to grab the data. In a [recent update of the engsoccerdata package](https://github.com/jalapic/engsoccerdata/commit/6133cf9f6fd77574a5a03097a6d2db4d213c508c) I added the location of grounds for teams in England which will let us find the distances teams have traveled to matches.

I also download a shapefile of the UK from [GADM](https://gadm.org/) for plotting and to filter out any bad data in ground location (which still is very much in beta).
...
The second method of calculating distance traveled takes the distance from each match _to_ the next match in run. I.e. it assumes a team stays in their past location until the next round (obviously not true in real life) and finds the distance to the ground for the next round game:
...
```
We can then find the routing distance using a nice trick I found on [Stack Overflow](https://github.com/r-spatial/sf/issues/799) to find the distance between each location and the next in the data.frame.
...
```{r find_run_routing_distances, warning=FALSE,message=FALSE}
#taken from
...
By functionalising our code from earlier, we can easily plot these well-traveled runs. Using the recently-added-to-CRAN [patchwork](https://github.com/thomasp85/patchwork) package, we can make multiple plots and stitch them together, e.g. for the 6 FA cup runs with the longest distances traveled we get:
...
["What's the largest town/city without a League club? I reckon Maidstone takes some beating (population 139,000 - about the same as Blackburn). Unlike Blackburn Rovers, Maidstone United play in the Kent League (of course Blackburn would not actually be eligible) which is some way below the Conference and Dr Martins Leagues. But being Maidstone United of course they play all their fixtures 12 miles away in Sittingbourne," writes Peter Driver. IN 2003](https://www.theguardian.com/football/2003/may/29/theknowledge.sport)

## Answer -

To answer this question, first we need data on towns and cities in England. We're going to rank by population so can download the population table found [here](http://lovemytown.co.uk/populations/TownsTable1.asp) to start with

```{r town_populations, warning=FALSE,message=FALSE}
...
To reproduce this script, you'll need to download the data from the [UK government achives](https://data.gov.uk/dataset/7879ab82-2863-401e-8a29-a56e264d2182/major-towns-and-cities-december-2015-boundaries) and point the file object towards it

```{r town_libraries, warning=FALSE,message=FALSE,echo=FALSE}
#to read in geojson data as an sf file
library(geojsonsf)

file <- "C:/Users/rob-getty/Downloads/Major_Towns_and_Cities_December_2015_Boundaries.geojson"
...
```{r fake_town_libraries, warning=FALSE,message=FALSE,eval = FALSE}
#to read in geojson data as an sf file
library(geojsonsf)
...
Then we want the club data. In the latest release of [engsoccer data](https://github.com/jalapic/engsoccerdata/pull/61) I added some (very beta) non-league data so we have a greater number of teams to pick from. We'll take the league the team played in in 2018 as this on league data hasn't been updated to 2019 (the current season) yet.

```{r towns_get_clubs, warning=FALSE,message=FALSE}
...
```{r find_town_clubs, warning=FALSE,message=FALSE}
...
So we can see that Gloucester and Basingstoke _do_ in fact have football teams, however a quick Wikipedia search shows that [they](https://en.wikipedia.org/wiki/Gloucester_City_A.F.C.) [both](https://en.wikipedia.org/wiki/Basingstoke_Town_F.C.) play outside their town (so I'm not sure if these count). The largest town* without a football team, down to the 8th tier of English football, is therefore Worcester with 100,000 people, but a team only in the [9th tier](https://en.wikipedia.org/wiki/Worcester_City_F.C.) of the football pyramid.

The question actually does specify _'League'_ teams, which generally only refers to the top 4 flights on English football. We can then run the function for each tier, finding the largest town without a team in that tier or above.

```{r town_clubs_by_tier, warning=FALSE,message=FALSE}
...
And the City of Coventry takes it, having a population of 325,000 and a team only in the 7th tier ([Bedworth United, who play just outside the city](https://en.wikipedia.org/wiki/Bedworth_United_F.C.)). This is only because the city's main team [Coventry City](https://en.wikipedia.org/wiki/Coventry_City_F.C.) are playing in Birmingham due to [ongoing difficulties finding a stadium within their own city](https://en.wikipedia.org/wiki/Coventry_City_F.C.#St_Andrew's). 
...
We can answer this using the directory of population places produced by the [Ordnance Survey](https://github.com/alasdairrae/gb-places). Again, download the .csv and point the script at it to reproduce.

```{r smallest_town_noecho, warning=FALSE,message=FALSE,echo=FALSE}
small_places <- read.csv("C:/Users/rob-getty/Downloads/gb_populated_places_all_BNG_csv/gb_populated_places_all_BNG.csv", stringsAsFactors = FALSE) %>%
...
```{r smallest_town_echo, warning=FALSE,message=FALSE,eval=FALSE}
...
```{r plot_small_places, warning=FALSE,message=FALSE}
...
```{r find_smallest_town_clubs, warning=FALSE}
...
Laneside is located just outside Accrington, where [Accrington Stanley](https://en.wikipedia.org/wiki/Accrington_Stanley_F.C.) play in the third tier (population 35,000).

Yeovil (population 45,000) is home to [Yeovil Town](https://en.wikipedia.org/wiki/Yeovil_Town_F.C.) who play in the 4th tier.

But the far and away winner is 4th tier [Forest Green Rovers](https://en.wikipedia.org/wiki/Forest_Green_Rovers_F.C.) who play in Nailsworth, with a population of just 5,700 people. Even more, the only remotely near town, Stroud, has a population of just 32,000 people. Most incredible of all, [the stadium for Forest Green Rovers](https://en.wikipedia.org/wiki/The_New_Lawn) can hold ~5,000 people, or almost the entire surrounding population.