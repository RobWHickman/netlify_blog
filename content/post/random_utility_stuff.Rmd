...
```{r data_directory, warning=FALSE,message=FALSE,include=FALSE}
data_directory <- "C:/Users/robwh/Desktop/rum_data"
```
...
notation from https://imai.fas.harvard.edu/teaching/files/discrete.pdf

Binary model such that
$$Y_{i} \in {0,1}$$
Where X is the difference in utility between the choices A and B on each trial, i

$$X_{i} = u(A_{i}) - u(B_{i})$$
using a logit model such that
$$\pi_{i} = \frac{1}{1 + e^{-\beta X_{i})}}$$
maximise the log likelihood
$$\mathcal l_{n}(\beta|Y,X) = \sum_{i=1}^{n} Y_{i} log(\pi_{i}) + (1-Y_{i})log(1-\pi_{i})$$ 
