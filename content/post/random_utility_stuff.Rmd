...
```{r generate_colleagues_data, warning=FALSE,message=FALSE,include=FALSE}
#seq over the range of 0-1 of slurm
colleagues_choice_data <- seq(0, 1, 0.2) %>%
  #generate distributions
  data.frame(x = .,
             a.y1 = 1-pnorm(., 0.30, 0),
             a.y2 = 1-pnorm(., 0.58, 0),
             a.y3 = 1-pnorm(., 0.95, 0),
             b.y1 = 1-pnorm(., 0.165, 0.24),
             b.y2 = 1-pnorm(., 0.33, 0.25),
             b.y3 = 1-pnorm(., 0.495, 0.23)) %>%
  #melt data
  gather("group", "dist", -x) %>%
  #separate out by person
  separate(group, into = c("person", "group"), sep = "\\.") %>%
  #rename data with our variables
  #buzz cola and slurm
  mutate(buzz_cola = case_when(
    group == "y1" ~ 1,
  ...
  mutate(person = case_when(
    grepl("a", person) ~ "Andreas",
    grepl("b", person) ~ "Béatrice"
  )) %>%
  mutate(slurm = x * 2000) %>%
  #round dist data
  mutate(fraction_choose_cola = round(dist, 2))  %>%
  select(person, buzz_cola, slurm, fraction_choose_cola)

#create some fake trial data by stretching this choice data
generate_colleagues_trial_data <- function(combination_row) {
  #for this combination how many times in cola chosen
  cola_choice <- round(combination_row$fraction_choose_cola*100)
  ...
  data.frame(buzz_cola = rep(combination_row$buzz_cola, 100),
             ...
             choice = c(rep("buzz_cola", cola_choice),
                        rep("slurm", 100 - cola_choice)),
             person = combination_row$person
             )
}

colleagues_trial_data <- colleagues_choice_data %>%
  split(f = seq(nrow(.))) %>%
  #apply the stretching function
  lapply(., generate_colleagues_trial_data) %>%
  #map together the data
  map_df(I) %>%
  #randomly shuffle the data
  .[sample(nrow(.)),]
...
```{r plot_colleagues_choices, warning=FALSE,message=FALSE}
...
```{r calculate_colleague_parameters, warning=FALSE,message=FALSE,include=FALSE}
colleagues_trial_data_binary <- colleagues_trial_data %>%
  mutate(buzz_cola_choice = case_when(
    choice == "buzz_cola" ~ 1,
    choice == "slurm" ~ 0
  )) %>%
  #list by person to lapply
  split(f = .$person)

colleague_parameters <- colleagues_trial_data_binary %>%
  lapply(., function(persons_choices) {
    optim(par = initial_parameters,
          #the functionise to optimise these over
          fn = log_likelihood_func,
          #other arguments to the function
          data = persons_choices,
          #optimisation algorithm to use
          method = "Nelder-Mead",
          #we are looking to maximise the sum
          #so fnscale set to -1
...
```{r colleague_optimal_parameters, warning=FALSE,message=FALSE}
...
```{r optimal_params, warning=FALSE,message=FALSE}
...
```{r calculate_colleague_utility_curves, warning=FALSE,message=FALSE,include=FALSE}
#slightly change function to allow lapply of peoples parameters
calc_likelihood <- function(parameters,
                            ratio_ba) {
  
  #get the single parameter values
  beta = parameters$par["beta"]
  rho_a = parameters$par["rho_a"]
  rho_b = parameters$par["rho_b"]
...
#calculate the utility curves per person
p6 <- colleague_parameters %>%
  lapply(., function(peronal_params) {
    x_seq <- seq(1/3, 3, by = 0.1)
    data.frame(ratio_ba = x_seq,
               likelihood_a = calc_likelihood(parameters = peronal_params, ratio_ba = x_seq))
  }) %>%
  #map together with id by person
  map_df(I, .id = "person") %>%
...
```{r plot_colleague_utility_curves, warning=FALSE,message=FALSE}
