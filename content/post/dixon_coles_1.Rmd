...
predictions <- predict_expg(model, fixtures$home, fixtures$away, return_df = TRUE) %>%
  mutate(hgoal = round(expg1, 0),
         agoal = round(expg2, 0)) %>%
...
```{r get_results, warning=FALSE,message=FALSE}
...
```{r results_matrix, warning=FALSE,message=FALSE}
...
```{r league_table_func, warning=FALSE,message=FALSE}
...
```{r league_table, warning=FALSE,message=FALSE}
...
```{r dc_model, warning=FALSE,message=FALSE}
library(goalmodel)
...
```{r goals_model, warning=FALSE,message=FALSE}
...
```{r check_result, warning=FALSE,message=FALSE}
results %>%
...
```{r run_simple_model, warning=FALSE,message=FALSE}
...
```{r print_actual_matrix, warning=FALSE,message=FALSE}
results_matrix
```
